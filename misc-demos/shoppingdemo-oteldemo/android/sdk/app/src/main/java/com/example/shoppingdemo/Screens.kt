...
import com.example.shoppingdemo.BuildConfig
...
fun WelcomeScreen(navController: NavController, simulationManager: SimulationManager) {
    ...
    ScreenContainer(
        ...
        imageUrl = "android.resource://${BuildConfig.APPLICATION_ID}/${R.drawable.opentelemetry_demo_logo}",
        ...
        if (simulationManager.isSimulating) {
            ...
            // Workshop 7 — Device Identification & Debugging (android, basic sdk)
            // workshop-301.md § "7 — Device Identification"
            // Device code: generates a short-lived code copied to clipboard for support lookup.
            Button(
                onClick = {
...
fun CartScreen(navController: NavController, productId: String?) {
    ...
    LaunchedEffect(pid) {
        try {
            ...
            // Workshop 4c — Logging (android, basic sdk)
            // workshop-301.md § "4c — Logging Examples" — log errors with throwable
...
fun CheckoutSignInScreen(navController: NavController, productId: String?) {
    ...
    LaunchedEffect(Unit) {
        try {
            ...
            // Add user_id on sign-in so every subsequent log is tagged with this user.
...
fun PaymentCardScreen(navController: NavController, checkoutSession: String?) {
    ...
    LaunchedEffect(session) {
        try {
            ...
            // Workshop 4c — Logging (android, basic sdk)
            // workshop-301.md § "4c — Logging Examples" — "payment_failed" with throwable
...
fun ConfirmationScreen(navController: NavController, orderId: String?) {
    ...
    ScreenContainer(
        ...
        Button(
            onClick = {
                // Workshop §3 (Initialization & Session Strategy): start a new session when the user
                // begins a new shopping journey — keeps each purchase flow in its own session.
                // https://docs.bitdrift.io/sdk/quickstart#android
                Logger.startNewSession()

                // Workshop 5 — Global Fields (android, basic sdk)
                // workshop-301.md § "5 — Global Fields"
                // Remove user_id when starting a new journey (acts as logout).
