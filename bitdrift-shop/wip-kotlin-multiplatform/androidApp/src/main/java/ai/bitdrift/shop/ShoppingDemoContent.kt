...
fun ShoppingDemoContent() {
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        NavHost(
            ...
            composable(
                route = AppScreen.PAYMENT_ANDROIDPAY_ROUTE,
                arguments = listOf(navArgument("checkoutSession") { type = NavType.StringType })
            ) { backStackEntry ->
                val checkoutSession = backStackEntry.arguments?.getString("checkoutSession")
            ...
            composable(
                route = AppScreen.PAYMENT_FAILED_ROUTE,
                ...
                PaymentFailedScreen(navController, paymentMethod)
            }
            composable(AppScreen.Advanced.route) {
                AdvancedScreen(navController, simulationViewModel)
            }
        }
