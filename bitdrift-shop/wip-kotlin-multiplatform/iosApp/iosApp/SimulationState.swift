...
class SimulationState: ObservableObject {
    ...
    // Variant & chaos state (polled from KMP engine)
    @Published var activeVariant: SimVariant = SimVariant.control
    @Published var crashLoopEnabled = false
    @Published var anrAEnabled = false
    ...
    func startAbSimulation() {
        let navigator = IOSNavigator(state: self)
        engine.startAbSimulation(
            navigator: navigator,
            scope: KotlinCoroutineScopeKt.createMainScope()
        )
        startPolling()
    ...
    func startCardinalitySimulation() {
        let navigator = IOSNavigator(state: self)
        engine.startCardinalitySimulation(
            navigator: navigator,
            scope: KotlinCoroutineScopeKt.createMainScope()
        )
        startPolling()
    ...
    func setVariant(_ variant: SimVariant) {
        engine.setVariant(v: variant)
        activeVariant = variant
    }

    func toggleCrashLoop() {
        let newVal = !engine.crashLoopEnabled.value.boolValue
        engine.setCrashLoop(b: newVal)
        crashLoopEnabled = newVal
    }

    func toggleAnrA() {
        let newVal = !engine.anrAEnabled.value.boolValue
        engine.setAnrA(b: newVal)
        anrAEnabled = newVal
    }

    func toggleForceQuit() {
        let newVal = !engine.forceQuitEnabled.value.boolValue
        engine.setForceQuit(b: newVal)
        forceQuitEnabled = newVal
    }
