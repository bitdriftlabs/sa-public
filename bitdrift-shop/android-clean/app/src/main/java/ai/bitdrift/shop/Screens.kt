...
fun WelcomeScreen(navController: NavController, simulationManager: SimulationManager) {
    ...
    LaunchedEffect(Unit) {
        ...
        try {
            AppWork.runSuspend("welcome_screen_load") {
                val welcome = AppWork.bestEffort { ApiClient.getWelcome() }
                apiData = welcome
                if (welcome == null) {
        ...
        if (simulationManager.isSimulating) {
            ...
            Button(
                onClick = {
                    ...
                    clipboardManager.setText(AnnotatedString(deviceCode!!))
                },
...
fun BrowseScreen(navController: NavController, simulationManager: SimulationManager? = null) {
    ...
    LaunchedEffect(Unit) {
        ...
        try {
          AppWork.runSuspend("browse_screen_load") {
...
fun ProductDetailScreen(navController: NavController, source: String?, productId: String?, simulationManager: SimulationManager? = null) {
    ...
    ScreenContainer(
        ...
        PrimaryButton(
            ...
            ScreenLogger.logInfo(mapOf("product_id" to pid, "source_screen" to (source ?: "unknown"))) { "add_to_cart" }
        ...
        SecondaryButton(
            ...
            ScreenLogger.logInfo(mapOf("product_id" to pid, "source_screen" to (source ?: "unknown"))) { "add_to_wishlist" }
...
fun CartScreen(navController: NavController, productId: String?) {
    ...
    LaunchedEffect(pid) {
        ...
        try {
            AppWork.runSuspend("cart_screen_load") {
                apiData = if (pid.isNotEmpty()) ApiClient.addToCart(pid) else ApiClient.getCart()
            }
        } catch (e: Exception) {

            ScreenLogger.logError(mapOf("product_id" to pid), throwable = e) { "cart_failed" }
        }
    }
    ...
    ScreenContainer(
                    ...
                    Card(
                        ...
                        Row(
                            ...
                            IconButton(onClick = {
                                ...
                                ScreenLogger.logInfo(mapOf("product_id" to itemProductId)) { "cart_item_removed" }
                                scope.launch {
        ...
        PrimaryButton(
            ...
            ScreenLogger.logInfo(mapOf("checkout_type" to "guest")) { "checkout_started" }
        ...
        SecondaryButton(
            ...
            ScreenLogger.logInfo(mapOf("checkout_type" to "signin")) { "checkout_started" }
...
fun CheckoutGuestScreen(navController: NavController, productId: String?) {
    ...
    LaunchedEffect(Unit) {
        ...
        try {
            AppWork.runSuspend("checkout_screen_load", mapOf("checkout_type" to "guest")) {
                apiData = ApiClient.checkoutGuest()
                checkoutSession = apiData?.optString("checkout_session", "") ?: ""
            }
        } catch (e: Exception) {

            ScreenLogger.logError(mapOf("checkout_type" to "guest"), throwable = e) { "checkout_failed" }
        }
    }
...
fun CheckoutSignInScreen(navController: NavController, productId: String?) {
    ...
    LaunchedEffect(Unit) {
        ...
        try {
          AppWork.runSuspend("checkout_screen_load", mapOf("checkout_type" to "signin")) {
            ...
            ScreenLogger.logError(mapOf("checkout_type" to "signin"), throwable = e) { "checkout_failed" }
        }
    }
...
fun PaymentCardScreen(navController: NavController, checkoutSession: String?) {
    ...
    LaunchedEffect(session) {
        ...
        try {
            AppWork.runSuspend("payment_screen_load", mapOf("payment_method" to "card")) {
                apiData = ApiClient.payCard(session)
                orderId = apiData?.optString("order_id", "") ?: ""
            }
        } catch (e: Exception) {

            ScreenLogger.logError(mapOf("payment_method" to "card"), throwable = e) { "payment_failed" }
        }
    }
    ...
    ScreenContainer(
        ...
        PrimaryButton(
            ...
            ScreenLogger.logInfo(mapOf("payment_method" to "visa", "card_last4" to "4242", "order_id" to orderId)) { "payment_completed" }
        ...
        SecondaryButton(
            ...
            ScreenLogger.logInfo(mapOf("payment_method" to "mastercard", "card_last4" to "8888", "order_id" to orderId)) { "payment_completed" }
            navController.navigate(Screen.Confirmation(orderId).route)
        }
        SecondaryButton(
            ...
            ScreenLogger.logInfo(mapOf("payment_method" to "amex", "card_last4" to "1001", "order_id" to orderId)) { "payment_completed" }
            navController.navigate(Screen.Confirmation(orderId).route)
        }
    }
}
...
fun PaymentApplePayScreen(navController: NavController, checkoutSession: String?) {
    ...
    LaunchedEffect(session) {
        ...
        try {
            AppWork.runSuspend("payment_screen_load", mapOf("payment_method" to "apple_pay")) {
                apiData = ApiClient.payApplePay(session)
                orderId = apiData?.optString("order_id", "") ?: ""
            }
        } catch (e: Exception) {

            ScreenLogger.logError(mapOf("payment_method" to "apple_pay"), throwable = e) { "payment_failed" }
        }
    }
    ...
    ScreenContainer(
        ...
        PrimaryButton(
            ...
            ScreenLogger.logInfo(mapOf("payment_method" to "apple_pay", "order_id" to orderId)) { "payment_completed" }
            navController.navigate(Screen.Confirmation(orderId).route)
        }
    }
}
...
fun PaymentPayPalScreen(navController: NavController, checkoutSession: String?) {
    ...
    LaunchedEffect(session) {
        ...
        try {
            AppWork.runSuspend("payment_screen_load", mapOf("payment_method" to "paypal")) {
                apiData = ApiClient.payPayPal(session)
                orderId = apiData?.optString("order_id", "") ?: ""
            }
        } catch (e: Exception) {

            ScreenLogger.logError(mapOf("payment_method" to "paypal"), throwable = e) { "payment_failed" }
        }
    }
    ...
    ScreenContainer(
        ...
        PrimaryButton(
            ...
            // this event, so omitting it here silently excluded these completions
            // from the conversion numbers.
            ScreenLogger.logInfo(mapOf("payment_method" to "paypal", "order_id" to orderId)) { "payment_completed" }
            navController.navigate(Screen.Confirmation(orderId).route)
        }
    }
}
...
fun PaymentAndroidPayScreen(navController: NavController, checkoutSession: String?) {
    ...
    LaunchedEffect(session) {
        ...
        try {
            AppWork.runSuspend("payment_screen_load", mapOf("payment_method" to "android_pay")) {
                apiData = ApiClient.payAndroidPay(session)
                orderId = apiData?.optString("order_id", "") ?: ""
            }
        } catch (e: Exception) {
            ScreenLogger.logError(mapOf("payment_method" to "android_pay"), throwable = e) { "payment_failed" }
        }
    }
    ...
    ScreenContainer(
        ...
        PrimaryButton(
            ...
            // this event, so omitting it here silently excluded these completions
            // from the conversion numbers.
            ScreenLogger.logInfo(mapOf("payment_method" to "android_pay", "order_id" to orderId)) { "payment_completed" }
            navController.navigate(Screen.Confirmation(orderId).route)
        }
    }
}
...
fun ConfirmationScreen(navController: NavController, orderId: String?) {
    ...
    LaunchedEffect(oid) {
        ...
        try {
            AppWork.runSuspend("confirmation_screen_load") {
