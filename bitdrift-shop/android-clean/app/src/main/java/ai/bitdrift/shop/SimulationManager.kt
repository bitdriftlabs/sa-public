...
class SimulationManager : ViewModel() {
    ...
    /** Repeatedly exercises the inventory lookup endpoint until cancelled. */
    ...
    private fun maybeFireCrash(nc: NavController) {
        ...
        ScreenLogger.logWarning { "about_to_crash: $name (context=$crashContext)" }
    ...
    fun fireFastCrash(activity: android.app.Activity?) {
        ...
        ScreenLogger.logWarning { "about_to_crash (fast): $name (context=$crashContext)" }
        ShoppingDemoApp.scheduleRestart(ctx, restartDelayFor(name))
        dispatchCrash(fn, fireInBackground, activity)
    }
    ...
    private suspend fun runSingleJourney(nc: NavController) {
        ...
        val journeySpan = AppOperation.start(
            name = "journey",
            level = AppOperationLevel.INFO,
        ...
        val discoverySpan = AppOperation.start(
        ...
        val discovered = AppWork.runSuspend(
        ...
        val forceQuitInjected = AppWork.runSuspend(
        ...
        if (Math.random() < wishlistProb) {
            ...
            AppWork.runSuspend("wishlist_add", parentSpanId = discoverySpan?.id) {
        ...
        discoverySpan?.end(AppOperationResult.SUCCESS, mapOf("source" to source, "product_id" to pid))
        ...
        AppWork.runSuspend("cart_assembly", parentSpanId = journeySpan?.id) {
        ...
        if (Math.random() < cartAbandonProb) {
            ...
            delay(200L)
            journeySpan?.end(AppOperationResult.CANCELED, mapOf("reason" to "cart_abandoned"))
            return
        }
        ...
        val checkoutSpan = AppOperation.start(
            name = "checkout",
            level = AppOperationLevel.INFO,
        ...
        if (Math.random() < checkoutDropoutProb) {
            ...
            delay(200L)
            checkoutSpan?.end(AppOperationResult.CANCELED, mapOf("reason" to "checkout_abandoned"))
            journeySpan?.end(AppOperationResult.CANCELED, mapOf("reason" to "checkout_abandoned"))
            return
        }
        ...
        val orderId: String = AppWork.runSuspend(
        ...
        if (willPaymentFail) {
            ...
            if (Math.random() < retryProb) {
                ...
                val retried = AppWork.runSuspend(
                ...
                AppWork.runSuspend("checkout.confirmation", parentSpanId = checkoutSpan?.id) {
                    try { ApiClient.getConfirmation(retryOrderId) } catch (ce: CancellationException) { throw ce } catch (_: Exception) {}
                }
                delay(200L)
                checkoutSpan?.end(AppOperationResult.SUCCESS, mapOf("payment_method" to retryMethod, "retried" to "true"))
                journeySpan?.end(AppOperationResult.SUCCESS)
                maybeFireCrash(nc)
            return
        }
        ...
        checkoutSpan?.end(AppOperationResult.SUCCESS, mapOf("payment_method" to paymentMethod))
        journeySpan?.end(AppOperationResult.SUCCESS)
        maybeFireCrash(nc)
    }
    }
