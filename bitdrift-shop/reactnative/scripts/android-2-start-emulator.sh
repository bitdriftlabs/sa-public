...
# default AVD ("Medium_Phone_Control") as that native app on purpose — see
# android-1-setup.sh's header comment for why (same applicationId, so the two
# apps already can't run side by side; no reason to keep separate AVDs).
set -euo pipefail
