#!/usr/bin/env bash
# Repeatedly foregrounds and backgrounds BitdriftShop, without a human
# switching apps by hand. Mirrors ../../ios/scripts/foreground-cycle.sh.
#
# Backgrounding uses demo-lib.sh's background_app (launches SpringBoard on a
# simulator, Settings on a device — neither platform lets an app background
# itself).
#
# Usage:
#   ./ios-foreground-cycle.sh          # cycle forever, until Ctrl-C
#   ./ios-foreground-cycle.sh -10      # cycle exactly 10 times
#   ./ios-foreground-cycle.sh --device -10
#   ./ios-foreground-cycle.sh --slow -5   # 30s foreground / 20s background per cycle,
