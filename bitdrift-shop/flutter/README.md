...
- ❌ ANR / force-quit / crash-loop injection — the alpha SDK has no Dart→native
  crash bridge, so those wouldn't be reported (intentionally omitted).
...
| Dart/Flutter exception → crash      | ❌ not prototyped → crash/ANR/force-quit omitted |
